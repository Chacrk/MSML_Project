...
class WRN(nn.Module):
    def __init__(self, args, way):
        ...
        self.use_conv_bias_shortcut = True
        self.use_conv_bias_normal = True
        ...
        # fc
        self.weights['fc/weight'] = nn.Parameter(torch.ones([way, 640]), True)
        self.weights['fc/bias'] = nn.Parameter(torch.zeros(way), True)
        ...
        self.task_lr['fc/weight'] = nn.Parameter(torch.tensor(args.lr_inner, dtype=torch.float32), requires_grad=True)
        self.task_lr['fc/bias'] = nn.Parameter(torch.tensor(args.lr_inner, dtype=torch.float32), requires_grad=True)

    def construct_block_weights(self, b, in_, out_):
        # expand
        self.weights['b{}_0/conv0/filter'.format(b)] = nn.Parameter(torch.ones([out_, in_, 3, 3]), False)
        ...
        self.weights['b{}_0/conv1/filter'.format(b)] = nn.Parameter(torch.ones([out_, out_, 3, 3]), False)
        if self.use_conv_bias_normal:
        ...
        for i in range(1, self.num_blocks):
            self.weights['b{}_{}/conv0/filter'.format(b, i)] = nn.Parameter(torch.ones([out_, out_, 3, 3]), False)
            if self.use_conv_bias_normal:
            ...
            self.weights['b{}_{}/conv1/filter'.format(b, i)] = nn.Parameter(torch.ones([out_, out_, 3, 3]), False)
            if self.use_conv_bias_normal:
    ...
    def forward(self, x, weights=None, bn_training=True, use_dropout=True):
        ...
        for block_index in range(1, 4):
            x = self.expand_conv(input_=x, weights=weights_use, b=block_index, bn_training=bn_training,
                                 use_dropout=use_dropout)
            for index_in_block in range(1, self.num_blocks):
                x = self.conv_block(input_=x, weights=weights_use, b=block_index, i=index_in_block,
                                    bn_training=bn_training, use_dropout=use_dropout)

        x = nn.AvgPool2d(10, stride=1)(x)
        x = x.view(x.size(0), -1)
        x = self.fc_forward(input_=x, weights=weights_use)
        return x
